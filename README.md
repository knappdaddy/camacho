...
Three alternative visual directions live alongside the site and can be compared
...
| **D** | Editorial — rebuilt from scratch against `BRIEF.md` | Newsreader + Schibsted Grotesk | warm paper, forest green, clay |

- <https://knappdaddy.github.io/camacho/theme-b/>
- <https://knappdaddy.github.io/camacho/theme-c/>
- <https://knappdaddy.github.io/camacho/theme-d/>

Each carries its own logo and favicon, and a bar at the top of the page
switches between all four on whichever page you're on.
...
**D is different.** It was rebuilt from nothing rather than layered on A: its
own markup, `theme-d/d.css` and `theme-d/d.js`, generated by
`tools/build-theme-d.py` from the same photos and content. Edit the generator,
not the HTML, then run `python3 tools/build-theme-d.py && python3 tools/stamp-build.py`.
Every direction the owner has given lives in `BRIEF.md`; check new work
against it.
